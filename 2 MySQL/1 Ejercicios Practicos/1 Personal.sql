...
SELECT nombre_jefe_depto FROM departamentos ORDER BY nombre_jefe_depto ASC;
...
SELECT nombre as NombreEmpleado, sal_emp as Salario, comision_emp as Comision FROM empleados WHERE id_depto =2000 ORDER BY comision_emp ASC;
...
SELECT nombre  as Nombre, (sal_emp+comision_emp+500) as Total_A_Pagar FROM empleados WHERE id_depto = 3000 ORDER BY nombre ASC;
...
SELECT nombre AS Nombre, sal_emp AS Salario, comision_emp AS Comision, (sal_emp+comision_emp) AS SalarioTotal FROM empleados WHERE comision_emp > 1000;
/*15. Obtener un listado similar al anterior, pero de aquellos empleados que NO tienen comisión.*/
SELECT nombre AS Nombre, sal_emp AS Salario, comision_emp AS Comision, (sal_emp+comision_emp) AS SalarioTotal FROM empleados WHERE comision_emp = 0;
/*16. Obtener la lista de los empleados que ganan una comisión superior a su sueldo.*/
SELECT nombre AS Nombre, sal_emp AS Salario, comision_emp AS Comision, (sal_emp+comision_emp) AS SalarioTotal FROM empleados WHERE comision_emp > sal_emp;
SELECT * FROM empleados WHERE comision_emp > sal_emp;
/*17. Listar los empleados cuya comisión es menor o igual que el 30% de su sueldo.*/
SELECT nombre AS Nombre, sal_emp AS Salario, comision_emp AS Comision, (sal_emp+comision_emp) AS SalarioTotal FROM empleados WHERE comision_emp < sal_emp*0.30;
...
SELECT nombre_DEPTO As Deparatameto FROM departamentos WHERE nombre_depto IN ('Ventas','Investigación','Mantenimiento');
/*20. Ahora obtener el contrario, los nombres de los departamentos que no sean “Ventas” ni “Investigación” ni ‘Mantenimiento.*/
SELECT nombre_DEPTO As Deparatameto FROM departamentos WHERE nombre_depto NOT IN ('Ventas','Investigación','Mantenimiento');
/*21. Mostrar el salario más alto de la empresa.*/
SELECT MAX(sal_emp ) AS SalarioMayor FROM empleados;
/*22. Mostrar el nombre del último empleado de la lista por orden alfabético.*/
SELECT MAX(nombre) AS NombreEmpelado FROM empleados ORDER BY nombre ASC;
...
SELECT round(AVG(sal_emp)) As SalarioPromedio FROM empleados;
				/*Consultas con Having*/
/*25. Hallar los departamentos que tienen más de tres empleados. Mostrar el número de empleados de esos departamentos.*/
SELECT id_depto AS 'Numero del Departamento', COUNT(id_emp) AS 'Cantidad Empleados' FROM empleados GROUP BY id_depto HAVING COUNT(id_emp) > 3;
...
SELECT nombre AS 'Nombre Empleado', cargo_emp AS 'Cargo', nombre_depto AS 'Nombre del departamento',nombre_jefe_depto AS 'Jefe del Departameto' FROM empleados INNER JOIN departamentos ON empleados.id_depto = departamentos.id_depto;
...
SELECT nombre AS 'Nombre Empleado', sal_emp AS 'Salario', cargo_emp AS 'Departamento' FROM empleados WHERE sal_emp >= (SELECT round(AVG(sal_emp)) FROM empleados) ORDER BY id_depto;
